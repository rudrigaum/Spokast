...
final class LibrarySyncService: LibrarySyncServiceProtocol {
    ...
    func syncMissingMetadata() async throws -> Int {
        ...
        let updates = await fetchMetadataInBatches(targets: targets, service: self.podcastService)
    ...
    nonisolated private func fetchMetadataInBatches(
        ...
        for startIndex in stride(from: 0, to: total, by: 1) {
            let end = min(startIndex + 1, total)
            let batch = Array(targetArray[startIndex..<end])
            let batchResults = await processBatch(batch, service: service)
    ...
    nonisolated private func processBatch(
        ...
        return await withTaskGroup(of: (Int, Podcast?).self) { group in
            for (id, data) in batch {
                group.addTask {
                    return await self.fetchSinglePodcastMetadata(
                        id: id,
                        url: data.url,
                        title: data.title,
                        service: service
    ...
    nonisolated private func fetchSinglePodcastMetadata(
        id: Int,
        url: String,
        title: String,
        service: PodcastServiceProtocol
